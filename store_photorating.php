<?php
	...
	if(!empty($id)){
		...
		if(!empty($rating)){
			$stmt = $conn->prepare("INSERT INTO vp_photoratings_1 (photoid, userid, rating) VALUES(?, ?, ?)");
			echo $conn->error;
		...
		$stmt = $conn->prepare("SELECT AVG(rating) as avgValue FROM vp_photoratings_1 WHERE photoid = ?");
		echo $conn->error;
